...
nadadorConMasAusencias: 'select  tu.username, tt.usuarioId ,count(presentismo) from t_turnos tt join t_usuarios tu on tt.usuarioId = tu.usuarioId where presentismo = 0 and fechaTurno < ? group by usuarioId order by count(presentismo) desc;'8uc  x,
